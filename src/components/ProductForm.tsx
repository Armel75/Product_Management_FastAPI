...
interface ProductFormProps {
  ...
  onAddCategory: (data: { name: string; description: string | null }) => Promise<Category>;
}

export const ProductForm: React.FC<ProductFormProps> = ({ categories, onSubmit, onAddCategory }) => {
  ...
  const handleCategoryCreate = async (e: React.MouseEvent) => {
    e.preventDefault();
  ...
  return (
    <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-xl space-y-6">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div>
          <label className="block text-slate-300 text-sm font-medium mb-1.5" htmlFor="root-product-name">
            Product Name *
          </label>
          <input
            id="root-product-name"
            type="text"
            required
        ...
        <div>
          <label className="block text-slate-300 text-sm font-medium mb-1.5" htmlFor="root-product-desc">
            Description
          </label>
          <textarea
            id="root-product-desc"
            rows={3}
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="block text-slate-300 text-sm font-medium mb-1.5" htmlFor="root-product-price">
              Price ($) *
            </label>
            <input
              id="root-product-price"
              type="number"
              step="0.01"
              min="0.01"
              required
          ...
          <div>
            <label className="block text-slate-300 text-sm font-medium mb-1.5" htmlFor="root-product-stock">
              Stock Quantity *
            </label>
            <input
              id="root-product-stock"
              type="number"
              min="0"
              required
              className="w-full px-4 py-2 bg-slate-950 border border-slate-800 rounded-xl text-slate-100 placeholder-slate-500 focus:outline-none focus:border-indigo-500 text-sm transition-colors"
        ...
        <div>
          <div className="flex justify-between items-center mb-1.5">
            <label className="text-slate-300 text-sm font-medium" htmlFor="root-product-category">
              Category
            </label>
            <button
              type="button"
